	...
	const BiProductScreen = (ProductScreen) =>
		class extends ProductScreen {
			...
			async _onClickPay() {
				...
				if(lines.length === 0){
				    self.showPopup('ErrorPopup', {title: self.env._t('Denied Order'),
										body: self.env._t('There is not single product in order.'),
								});
                    return
				}
                ...
                var orderProductIds = []
                var accepted = false
                var price_greater = false
                for (var i = 0; i < arrayLength; i++) {
                    console.log(lines[i],4444444444444)
                ...
                if (accepted == false){
                    self.showPopup('ErrorPopup', {title: self.env._t('Denied Order'),
                                            body: self.env._t('There is not single product which have non zero quantity.'),
                                    });
                        return
                }
                if (price_greater == false){
                    self.showPopup('ErrorPopup', {title: self.env._t('Denied Order'),
                                            body: self.env._t('There is not single product which have non zero price.'),
                                    });
                        return
                }
							...
							if(flag > 0){
								...
								self.showPopup('ErrorPopup', {
									title: self.env._t('Denied Order'),
										body: self.env._t('Ordered qty of One or more product(s) '+ product +' is more than available qty.'),
								});
							}
						});
					} else {
						$.each(lines, function( i, line ){
							if (line.product.type == 'product'){
								if (allow_order == false && line.quantity > line.product['bi_on_hand']){
									...
									self.showPopup('ErrorPopup', {
										title: self.env._t('Denied Order'),
										body: self.env._t('Ordered qty of One or more product(s) '+ line.product.name +' is more than available qty.'),
									});
									return
								}
