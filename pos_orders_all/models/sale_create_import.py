...
class InheritPOSOrder(models.Model):
    ...
    def create_deliver_date(self, partner_id,cashier_id, delivery_date):
        delivery_date = self.env['sale.order'].browse(int(delivery_date))
        test_order = self.env['sale.order'].create({
            'partner_id': partner_id,
            'user_id': cashier_id,
            'delivery_date': delivery_date.id,
        })
        return test_order



    def create_sales_order(self, partner_id, orderlines, cashier_id, delivery_date):
...
class SaleOrderLine(models.Model):
    ...
    date_order = fields.Char('date_order')
