...
def export_generic_method_for_invoice(sheet_name, columns):
...
class EbizUploadInvoices(models.Model):
    ...
    invoice_lines = fields.One2many('ebiz.list.invoice', 'sync_invoice_id', string=" ", copy=True,)
    logs_line = fields.One2many('ebiz.log.invoice', 'sync_log_id', string=" ", copy=True,)

    def create_default_records(self):
        list_of_invoices = self.env['account.move'].search([("move_type", "=", "out_invoice")])
        list_of_upload_invoice = self.env['ebiz.list.invoice'].search([])
        if (len(list_of_invoices)) != (len(list_of_upload_invoice)):
            list_of_dict = []
            for invoice in list_of_invoices:
                is_order = self.env['ebiz.list.invoice'].search([('invoice', '=', invoice.id)])
                if not is_order:
                    list_of_dict.append({
                        'invoice': invoice.id,
            ...
            if list_of_dict:
                self.env['ebiz.list.invoice'].create(list_of_dict)

    def read(self, fields):
        self.create_default_records()

        resp = super(EbizUploadInvoices, self).read(fields)
        return resp

    def upload_invoice(self,  *args, **kwargs):
        try:

            filter_record = self.env['ebiz.list.invoice'].browse(kwargs['values']).exists()
    ...
    def export_orders(self,  *args, **kwargs):
        ...
        worksheet, workbook, header_style, text_center = export_generic_method_for_invoice(sheet_name='Invoices',
        ...
        export_id = self.env['bill.excel'].create(
            {'excel_file': base64.encodestring(fp.getvalue()), 'file_name': 'Invoices.xls'})
    ...
    def export_logs(self,  *args, **kwargs):
        filter_record = self.env['ebiz.log.invoice'].browse(kwargs['values']).exists()
        if not filter_record:
            raise UserError('Please select a record first!')
        ...
        worksheet, workbook, header_style, text_center = export_generic_method_for_invoice(sheet_name='Invoice Logs',
                                                                               columns=column_names)
        i = 4
...
class EbizListInvoie(models.Model):
...
class EbizListLog(models.Model):
    ...
    sync_log_id = fields.Many2one('ebiz.upload.invoice', string='Partner Reference', required=True,
                                          ondelete='cascade', index=True, copy=False)
