...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _get_partner_id(self):
        if 'active_id' in self.env.context:
            return self.env[self.env.context.get('active_model')].browse(self.env.context.get('active_id')).partner_id.id
        else:
            return self.env[self.env.context.get('active_model')].browse(self.env.context.get('active_ids')[0]).partner_id.id
    ...
    sub_partner_id = fields.Many2one('res.partner', "Sub Partner Id", default=_get_partner_id)
    ...
    #         super(AccountPayments, self).post()
    ...
    #             if not self.card_id:
    #                 my_card_no = self.card_card_number
    #                 self.new_card = True
    ...
    #                 self.payment_token_id = token_id
    #         elif self.env.context.get('pass_validation'):
    ...
    #         if self.card_id:
    #             self.card_id.card_code = self.security_code
    #
    #         if config.get(
    #                 'merchant_card_verification') == 'full-amount' and self.token_type == 'credit' and not self.card_id:
    #             # if True:
    #             resp = transactions.with_context({'run_transaction': True}).s2s_do_transaction(
    #                 **{'command': command, 'card': my_card_no})
    #         else:
    ...
    #                     res = super(AccountPayments, self).post()
    ...
    #     su = super(AccountPaymentRegister, self)._onchange_set_payment_token_id()
    #
    #     if self.invoice_ids and self.invoice_ids[0].type == "out_refund" and self.journal_id == journal_id:
    #         trans_ids = self.invoice_ids.reversed_entry_id.transaction_ids
    #         if trans_ids:
    ...
    #             self.payment_transaction_id.write({'is_processed': True})
    ...
    def js_flush_customer(self, *args, **kwargs):
        if kwargs['customers'] and kwargs['customers'][0] == None:
            ...
            if customer:
                customer.payment_token_ids.filtered(lambda x: x.create_uid == self.env.user).with_context(
                    {'donot_sync': True}).unlink()
    ...
    def _compute_journal_code(self):
        ...
        journal_id = acquirer.journal_id
        for payment in self:
            if payment.journal_id.id == journal_id.id and payment.line_ids:
    ...
    def _post_payments(self, to_process, edit_mode=False):
        ...
        if self.journal_code == 'EBIZC':
            responce_check = payments.with_context({'payment_data': {
                'token_type': self.token_type,
                'card_id': self.card_id,
                'ach_id': self.ach_id,
                'card_card_number': self.card_card_number,
                'card_card_code': self.card_id.card_code,
                ...
                'to_reconcile': int(to_process[0]['to_reconcile'].id) or False,
            ...
            if 'xml_id' in responce_check:
                if responce_check['xml_id'] == 'payment_ebizcharge.action_ebiz_transaction_validtion_form':
    ...
    def _create_payments(self):
        ...
        #     if responce_validation['xml_id'] == 'payment_ebizcharge.action_ebiz_transaction_validtion_form':
    ...
    def action_create_payments(self):
        if self.can_group_payments and self.env.ref('payment_ebizcharge.payment_acquirer_ebizcharge').journal_id.name == self.journal_id.name:
            raise UserError('EBizCharge is currently disabled for group/bulk payments!')

        payments = self._create_payments()

        # if 'xml_id' in payments:
        #     if payments['xml_id'] == 'payment_ebizcharge.action_ebiz_transaction_validtion_form':
        #         return payments
