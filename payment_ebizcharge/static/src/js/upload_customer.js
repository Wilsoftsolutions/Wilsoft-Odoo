odoo.define('payment_ebizcharge.upload_customer', function (require) {
    "use strict";
    ...
    var CustomRendererTwo = WidgetViewer['CustomRenderer'].extend({
        ...
        'click .button-delete': '_onCustomDeleteButtonClicked',
        }, WidgetViewer['CustomRenderer'].prototype.events),
        _renderView: function () {
            return this._super.apply(this, arguments).then(() => {
                if(this.state.model === 'list.of.customers'){
                    var buttonName = '<div class="float-right my-3"><button type="button" class="btn btn-primary button-abc">Upload</button><button type="button" class="ml-2 btn btn-primary button-delete">Deactivate</button><button type="button" class="ml-2 btn btn-primary button-export">Export</button></div>'
                }

                else if(this.state.model === 'logs.of.customers'){
        ...
        _onCustomDeleteButtonClicked: function (ev) {
            var self = this;
            var res_ids = [];
            var hash = window.location.hash.substring(1);
            var params = $.deparam(hash);
    ...
    fieldRegistry.add('upload_customer', FieldVideoPreview);
