...
class StockMovementReport(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        for uniq_product in uniq_product_list:
            ...
            #Opening Stock  
            opening_qunats = quantsa.search([('product_id','=',uniq_product),('in_date','<',docs.date_from)])
            quants = [op_qt.quantity for op_qt in opening_qunats]
            within_quantsa = quantsa.search([('product_id','=',uniq_product),('in_date','>=',docs.date_from),('in_date','<=',docs.date_to)])
